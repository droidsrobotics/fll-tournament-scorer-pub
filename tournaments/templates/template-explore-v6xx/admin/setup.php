...
document.write('<a href="/upgrade.php#'+textFileToArray('/' + window.location.href.split('/')[3] + '/tournament.txt')[0]+'">Upgrade Assistant</a><br>');
            ...
            if ($_POST["sendemail"] == "on") {
              ...
              if (!file_exists("../userdata/" . strtolower($teamuser))) {
                ...
                if (strpos($_SERVER['SERVER_NAME'], "virtualopeninvitational") !== false) {
                  // echo "hello";
                  // echo 'printf "To: ' . $teamuser . '\nFrom: FLL Tutorials Tournament\nSubject: Virtual Open Invitational Tournament Invite\nWelcome to ' . $tournament . ' tournament. Your FLL tournament director has invited you as team ' . $teamno . '. Please create an account at tournament.virtualopeninvitational.org with your email address. You will then be able to log into the tournament from your account." | msmtp ' . $teamuser . '' . "<br>";


                  exec('printf "To: ' . $teamuser . '\nFrom: FLL Tutorials Tournament\nSubject: Virtual Open Invitational Tournament Invite\nWelcome to ' . $tournament . ' tournament. Your FLL tournament director has invited you as team ' . $teamno . '. Please create an account at tournament.virtualopeninvitational.org with your email address. You will then be able to log into the tournament from your account." | msmtp ' . $teamuser . '');
                } else {
                  exec('printf "To: ' . $teamuser . '\nFrom: FLL Tutorials Tournament\nSubject: FLLTutorials Tournament Invite\nWelcome to ' . $tournament . ' tournament. Your FLL tournament director has invited you as team ' . $teamno . '. Please create an account at tournament.flltutorials.com with your email address. You will then be able to log into the tournament from your account." | msmtp ' . $teamuser . '');
                }
              } else {
        ...
        <p>Instructions: Enter your list of teams attending this tournament. List the teams in the order you want them to appear on the Rubric Manager that the Judges will use. Remember to hit the "Save" button after this step.</p>
        <br>
        <pre>Format: Team No.,Team Name,coach email,coach email 2,judging lane</pre>
        <form action="" method="post">
          <textarea class="form-control" style="font-family:Consolas,Monaco,Lucida Console,Liberation Mono,DejaVu Sans Mono,Bitstream Vera Sans Mono,Courier New, monospace;" placeholder="51,Not the Droids You Are Looking For,team@droidsrobotics.org,team@ev3lessons.com,1" rows="10" cols="50" id="text" name="text"><?php echo htmlspecialchars($text) ?></textarea><br>
...
team no. |  team name  |    coach/team email        | coach/team email 2    |  Judging Lane |   <== Do not include this row
----------------------------------------------------------------------------------------------
1        |   sample    |    user@example.com        |  user2@example.com    |      1        |
51       |   droids    |    team@droidsrobotics.org |  team@ev3lessons.com  |      2        | 
</pre>
...
-------------------------------------------------------
 ...
 None                      |  https://drive.google.com -->
        ...
        <p> You can have an alternate user(s) for the head referee and referees. Use the <button class="btn btn-danger" onclick="window.location.href = 'createuser.php?data=' + window.location.href.split('/')[3]">Create Other User(s)</button> wizard to add user(s) if you have not yet.</p>
        <br>
        <br>
        <b>STEP 3 (OPTIONAL): VIRTUAL TEAM PITS</b><br>
...
-------------------------------------------------------
 ...
 None                      |  https://drive.google.com -->
        ...
        ?>
        <form action="" method="POST">
          <b>Access Code for Pits (leave blank for public)</b>
        ...
        <br><b style='font-size:16px;color:red;'>
          <?php
          $latest = trim(file_get_contents("../../template-explore/version.txt"));
