...
var LOCATIONS = {
    ...
    getLocation: function (id, loc, num = 0) {
        ...
        name = arr[loc];
        if (name == null) {
            if (id <= 1) name = this.names[id - 1] + " " + (loc + 1);
            else {
...
function generate() {
    if ((SCHEDULE != null && SCHEDULE.error == 0) || String(document.getElementById("results").innerHTML).trim() != "") {
...
function readInputs() {
    ...
    var tp = new TeamParams(nTeams, minTravel),
        ...
        rpJudge = new JudgeParams(j1Start, j1End, n1Judges, j1Length, j1Buffer, "Judging", 1),
...
function printSchedule(scheduler) {
    ...
    if (scheduler.error > 0) {
        ...
        $("#saveBtn").get(0).style.display = "none";
        ...
        $("#saveBtn").get(0).style.display = "inline-block";
...
function printIndivTeams(scheduler) {
    ...
    var table = document.createElement("TABLE");
    table.appendChild(newElement("TH", "Team No."));
    ...
    for (var i = 0; i < scheduler.judgeList.length; i++) {
        ...
        table.appendChild(newElement("TH", scheduler.judgeList[i].name + " location"));
    }
    for (var i = 0; i < scheduler.matchList.length; i++) {
        ...
        table.appendChild(newElement("TH", scheduler.matchList[i].name + " location"));
    }
