...
function fileExists(file_url){
  ...
  function gentable(teamin) {
      ...
      // console.log("../teams/" +fwteam+ "/robotScores/" +c+ ".txt")
	      teamscore = textFileToArray("../teams/" +fwteam+ "/robotScores/" +c+ ".txt")[0].split(';')[0];
	  } else {
	      teamscore = 0
	  }
	  //document.write(' <td id="'+fwteam+'-'+c+'"><a href="/scorer/index.html?tourn='+tourn+'&team='+fwteam+'&round='+c+'">'+String(teamscore)+'</a></td>');
	  document.write(' <td id="'+fwteam+'-'+c+'"><button style="background-color:lightblue;width:100%; height:60px; font-size:40px;" onclick="window.parent.location.href = \'index.php?tourn='+tourn+'&team='+fwteam+'&round='+c+'\'">'+String(teamscore)+'</button></td>');
