...
if (isset($_POST['text']))
{
    ...
    foreach ($textAr as $line) {
      ...
      if (!file_exists("../userdata/" . $teamuser)) {
        file_put_contents("../userdata/" . $teamuser, $teamno);

        // exec('printf "To: '.$teamuser.'\nFrom: noreply@ev3lessons.com\nSubject: Invitation for FLL Challenge Tournament Scoring System\nWelcome to '.$tournament.' tournament. Your FLL Challenge tournament director has invited you as team '.$teamno.'. Please create an account at tournament.flltutorials.com with your email address. You will then be able to log into the tournament from your account." | msmtp '.$teamuser.'');
      } else {

        $user_pass = fopen("../userdata/" . $teamuser, "r");
        $flag = 0;
        ...
        if ($flag == 0) {
          $current = file_get_contents("../userdata/" . $teamuser);
          $current .= "\n".$teamno;
          file_put_contents("../userdata/" . $teamuser, $current);
        }
...
<form action="" method="post">
      <textarea class="form-control" placeholder="1,sample,user@example.com" rows="10" cols="50" id="text" name="text"><?php echo htmlspecialchars($text) ?></textarea><br>
...
File should be .csv with comma as the field delimeter. There should be no column headers. Format in Excel should be: 
<pre style="font-family: monospace;">
team no. |  team name  |    coach/team email  <== Do not include this row
------------------------------------------------
1        |   sample    |    user@example.com
51       |   droids    |    team@droidsrobotics.org
</pre>
Export as CSV (comma delimited) to work in this system.
</div>
...
Instructions: This cannot be undone. Once you release rubrics/scores, teams will be able to view them by logging into this tournament.<br>
<?php
...
You can have an alternate user(s) for the head referee and referees. Use the <button class="btn btn-danger" onclick="window.location.href = 'createuser.php?data=' + window.location.href.split('/')[3]">Create Other User(s)</button> wizard to add user(s) if you have not yet.
<br>
